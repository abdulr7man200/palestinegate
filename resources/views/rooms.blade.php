...
<style>
  ...
  .search-container {
    ...
    width: 90%;
    max-width: 850px;
    margin: 20px auto; /* Centers it without absolute positioning */
}
  .search-bar {
      display: flex;
      flex-wrap: nowrap;
          ...
          <form action="{{ route('showrooms', request()->id) }}" method="GET">
            <div class="search-container">
              <form action="{{ route('showrooms', request()->id) }}" method="GET">
                  ...
                  <div class="filters">
                      <label>
                          <input type="checkbox" name="has_ac" {{ request('has_ac') == 'on' ? 'checked' : '' }}> Air Conditioning
                      </label>
                      <label>
                          <input type="checkbox" name="has_tv" {{ request('has_tv') == 'on' ? 'checked' : '' }}> TV
                      </label>
                      <label>
                          <input type="checkbox" name="has_wifi" {{ request('has_wifi') == 'on' ? 'checked' : '' }}> Wi-Fi
                      </label>
                  </div>
